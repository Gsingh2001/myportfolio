...
export default function AstraiosDetail({ project }: { project: any }) {
    ...
    <>
      ...
      <section className={section}>
        <div className={containerNarrow}>
          <p className={sectionTitle}>3D Trajectory Overview</p>
          <h2 className={heading}>All Moon_* sequences combined.</h2>
          <p className="mt-3 max-w-3xl text-sm text-[#5a5a66]">
          ...
          <div className="mt-8 rounded-3xl border border-[#eeeef0] p-4 md:p-6 bg-white">
      ...
      <section className={section}>
        <div className={containerNarrow}>
          <p className={sectionTitle}>Results — Aggregate</p>
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-5 gap-3">
            {stats.map((s) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-[#71717f] leading-snug uppercase tracking-wider">
                  {s.label}
                </p>
              </div>
            ))}
      ...
      <section className={section}>
        <div className={containerNarrow}>
          <p className={sectionTitle}>Results — Per Scene</p>
          <h2 className={heading}>GT vs. QXMT-SLAM trajectories.</h2>
          <p className="mt-4 max-w-3xl text-[#5a5a66]">
          ...
          <div className="mt-10 grid md:grid-cols-2 gap-5">
            {scenes.map((s: any) => (
              <figure
                key={s.name}
                className="rounded-2xl border border-[#eeeef0] p-4 bg-white hover:border-[#d8d8de] transition-colors"
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={s.image}
                ...
                <figcaption className="mt-3 flex items-baseline justify-between text-sm">
                  <span className="font-semibold text-[#1a1a1f]">Scene {s.name}</span>
                  <span className="text-xs text-[#71717f] font-mono">
      ...
      <section className={section}>
        <div className={containerNarrow}>
          <p className={sectionTitle}>Architecture</p>
          ...
          <div className="mt-8 rounded-3xl border border-[#eeeef0] p-6 md:p-10 bg-white">
            <div className="grid md:grid-cols-5 gap-4 text-center text-sm">
              {[
                ...
                <div key={m.t} className="md:col-span-1 rounded-xl border border-[#d8d8de] p-4">
                  <p className="text-xs uppercase tracking-wider text-[#71717f]">Modality</p>
                  <p className="mt-1 font-semibold text-[#1a1a1f]">{m.t}</p>
                  <p className="text-xs text-[#71717f] mt-1">{m.s}</p>
                </div>
              ))}
              <div className="md:col-span-2 rounded-xl bg-[#1a1a1f] text-white p-4">
                <p className="text-xs uppercase tracking-wider text-[#b6b6c0]">Fusion</p>
                <p className="mt-1 font-semibold">
                  Transformer Encoders + Quantum-Inspired Cross-Attention
                </p>
                <p className="text-xs text-[#b6b6c0] mt-1">
            ...
            <div className="mt-4 grid md:grid-cols-2 gap-4 text-sm">
              <div className="rounded-xl border border-[#d8d8de] p-4">
                <p className="text-xs uppercase tracking-wider text-[#71717f]">
                  Temporal Modelling
                </p>
                <p className="mt-1 font-semibold text-[#1a1a1f]">LSTM over fused tokens</p>
